...
# Eklentileri Aktifleştirme (Oturum yeniden başlatıldığında devreye girmesi için UUID ile enable komutu)
bilgi "Eklentiler aktifleştiriliyor..."
bilgi "Kurulu GNOME eklentileri sorgulanıyor..."
KURULU_EKLENTILER=()
if command -v gnome-extensions &> /dev/null; then
    while IFS= read -r line; do
        KURULU_EKLENTILER+=(\"$line\")
    done < <(gnome-extensions list 2>/dev/null || true)
fi

# Etkinleştirilecek eklenti UUID'leri
EKLENTI_UUIDLER=(
...
for uuid in "${EKLENTI_UUIDLER[@]}"; do
    if [[ \" ${KURULU_EKLENTILER[@]} \" =~ \" ${uuid} \" ]]; then
        ...
        uyari \"$uuid kurulu değil, atlanıyor.\"
    fi
done
